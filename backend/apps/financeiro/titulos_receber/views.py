...
class TituloReceberViewSet(viewsets.ModelViewSet):
    ...
    def baixa_parcial(self, request, pk=None):
        ...
        if valor_via_carteira > 0 and titulo.cliente_externo_id:
            return Response(
                {"detail": "Pagamento via carteira não é permitido para clientes externos."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        titulo.aplicar_baixa_parcial(
            valor=ser.validated_data["valor"],
            juros=ser.validated_data.get("juros", Decimal("0")),
