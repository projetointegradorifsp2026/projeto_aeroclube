...
export default function Clientes() {
  const [clientes, setClientes] = useState<Entidade[]>([])
  ...
  async function handleSave(data: EntidadeFormData) {
    if (editItem) {
      ...
      setClientes(prev => prev.map(e => (e.id === editItem.id ? updated : e)))
    } else {
    ...
    <div className="pt-2 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          Gerencie os clientes do aeroclube
        </p>
      </div>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-14 text-muted-foreground">
              ...
              <p className="text-xs mt-1">Tente ajustar os filtros ou cadastre um novo cliente</p>
            </div>
          ) : (
